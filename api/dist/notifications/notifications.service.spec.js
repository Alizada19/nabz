...
describe('NotificationsService', () => {
    ...
    beforeEach(() => {
        ...
        service = new notifications_service_1.NotificationsService(prisma, fcmProvider);
    });
    it('creates a notification and dispatches a push message', async () => {
        ...
        expect(fcmProvider.send).toHaveBeenCalledWith(expect.objectContaining({ userId: 'u1' }));
    });
    it('notifyMany batches persistence and push dispatch for multiple donors', async () => {
        ...
        expect(fcmProvider.sendBatch).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ userId: 'u1' })]));
    });
    ...
    it('throws NotFoundException when marking a non-existent notification as read', async () => {
        ...
        await expect(service.markAsRead('n404', 'u1')).rejects.toThrow(common_1.NotFoundException);
    });
});

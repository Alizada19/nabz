...
describe('BloodCompatibilityService', () => {
  ...
  it('O+ can receive from O+ and O-', () => {
    expect(service.getCompatibleDonorTypes('O+').sort()).toEqual(
      ['O+', 'O-'].sort(),
    );
  });
  ...
  it('AB+ is the universal recipient (accepts all types)', () => {
    ...
    expect(result.sort()).toEqual(
      ['AB+', 'AB-', 'A+', 'A-', 'B+', 'B-', 'O+', 'O-'].sort(),
    );
  });

  it('A+ can receive from A+, A-, O+, O-', () => {
    expect(service.getCompatibleDonorTypes('A+').sort()).toEqual(
      ['A+', 'A-', 'O+', 'O-'].sort(),
    );
  });

  it('B- can receive from B- and O- only', () => {
    expect(service.getCompatibleDonorTypes('B-').sort()).toEqual(
      ['B-', 'O-'].sort(),
    );
  });

  it('AB- can receive from AB-, A-, B-, O-', () => {
    expect(service.getCompatibleDonorTypes('AB-').sort()).toEqual(
      ['AB-', 'A-', 'B-', 'O-'].sort(),
    );
  });
  ...
  it('normalizes case and whitespace', () => {
    expect(service.getCompatibleDonorTypes(' o+ ')).toEqual(
      expect.arrayContaining(['O+', 'O-']),
    );
  });
  ...
  it('lists all 8 valid blood types', () => {
    expect(service.getAllValidBloodTypes().sort()).toEqual(
